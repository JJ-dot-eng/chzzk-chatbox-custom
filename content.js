(() => {
  ...
  function findSettingsButtonTarget() {
    ...
    const leftSideEnd = headerRect.left + headerRect.width * 0.34;
    const headerActions = queryAllSafe(header, CHAT_HEADER_ACTION_BUTTON_SELECTORS);
    const actions = headerActions
      .filter((element) => element instanceof HTMLElement)
      .filter((element) => element.id !== SETTINGS_BUTTON_ID && element.id !== GUEST_CHAT_TOGGLE_BUTTON_ID)
      .filter((element) => !element.closest(`#${SETTINGS_BUTTON_ID}, #${GUEST_CHAT_TOGGLE_BUTTON_ID}`))
      .filter(isElementVisible)
      .filter((element) => !isNestedHeaderAction(element, headerActions))
      .map((element) => ({ element, rect: element.getBoundingClientRect() }))
      .filter(({ rect }) => rect.width > 0 && rect.height > 0)
      .sort((left, right) => left.rect.left - right.rect.left || left.rect.top - right.rect.top);
    const leftAction = actions.find(({ rect }) => rect.left <= leftSideEnd)?.element || null;
    const fallbackChild = [...header.children]
      .filter((element) => element instanceof HTMLElement)
      .filter((element) => element.id !== SETTINGS_BUTTON_ID && element.id !== GUEST_CHAT_TOGGLE_BUTTON_ID)
      .filter(isElementVisible)
      .map((element) => ({ element, rect: element.getBoundingClientRect() }))
      .filter(({ rect }) => rect.width > 0 && rect.height > 0)
      .sort((left, right) => left.rect.left - right.rect.left || left.rect.top - right.rect.top)[0]?.element || null;
    const anchor = leftAction || fallbackChild;
    const container = anchor?.parentElement instanceof HTMLElement ? anchor.parentElement : header;
    const existingButton = getSettingsButton();
    const nextSibling = anchor?.nextSibling === existingButton
  ...
  function setSettingsButtonState(button = getSettingsButton(), state = "idle") {
  ...
  function resetSettingsButtonStateLater(button) {
    window.setTimeout(() => {
  ...
  function openSettingsPopupFallback() {
    const runtime = getRuntime();
    ...
    return Boolean(
      window.open(
        popupUrl,
        "chzzkChatUiToggleSettings",
        "popup=yes,width=340,height=620,noopener"
      )
    );
  }
  ...
  function openSettingsPopupFromHeader(button) {
    ...
    setSettingsButtonState(button, "loading");

    if (!runtime?.runtime?.sendMessage) {
      const opened = openSettingsPopupFallback();
      setSettingsButtonState(button, opened ? "idle" : "error");
      resetSettingsButtonStateLater(button);
      return;
    }

    try {
      runtime.runtime.sendMessage({ type: OPEN_SETTINGS_POPUP_MESSAGE }, (response) => {
        const error = runtime.runtime?.lastError;
  ...
  function createSettingsButton() {
    ...
    button.addEventListener("click", (event) => {
      ...
      openSettingsPopupFromHeader(button);
    });
